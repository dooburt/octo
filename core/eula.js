...
class Eula {
  ...
  */
  ...
  async getUrl(serverPath) {
    this.url = (await fs.readFile(path.join(serverPath, this.file), "utf8")).match(Eula.regexp.url)[0];
    return this.url;
  }
  ...
  async check(serverPath) {
    return JSON.parse((await fs.readFile(path.join(serverPath, this.file), "utf8")).match(Eula.regexp.state)[0]);
  }
